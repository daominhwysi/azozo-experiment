...
from backend.app.domains.ocr.parser.parser import parse_spans_into_structured_questions, regex_parse_questions
from backend.app.domains.ocr.parser.ocr_logger import log_ocr_annotate_request
...
async def parse_exam_from_pdf_or_text(
    ...
    try:
        ...
        annotation_res = annotator.annotate_text(
            ocr_text,
        ...
        def run_annotation():
            ...
            try:
                ...
                try:
                    ...
                    print(
                        f"[Fallback Parser in Stream] Stream annotation failed ({stream_error}), retrying with non-stream annotation."
                    )
                    annotation_res = annotator.annotate_text(
                        ocr_text,
...
def run_ocr_task_bg(
    ...
    import time
    from backend.app.core.database import load_db, save_db
    from backend.app.domains.ocr.annotator.pdf_converter import PDFOCRConverter
    from backend.app.domains.ocr.annotator.annotate_ocr import OCRAnnotator
    from backend.app.domains.ocr.parser.parser import parse_spans_into_structured_questions, regex_parse_questions
    from backend.app.core.config import OCR_MODEL, OCR_BATCH_SIZE, OCR_CONCURRENCY, PARSER_MODEL
    from backend.app.domains.ocr.parser.ocr_logger import log_ocr_annotate_request
    ...
    try:
        ...
        annotator = OCRAnnotator(model=PARSER_MODEL, provider=PARSER_PROVIDER)
        bg_request_id = uuid.uuid4().hex[:10]
        try:
            annotation_res = annotator.annotate_text_stream(
                ocr_text,
                callback=bg_token_callback,
...
async def parse_exam_long_context(
    ...
    try:
        ...
        pipeline = LongContextParserPipeline(
            ...
            linker_provider=LINKER_PROVIDER,
            batch_size=OCR_BATCH_SIZE,
            concurrency=OCR_CONCURRENCY,
        )
        loop = asyncio.get_running_loop()
        res = await loop.run_in_executor(None, pipeline.parse_pdf_long_context, str(temp_pdf_path))
        return res
    except Exception as e:
        import traceback
        traceback.print_exc()
