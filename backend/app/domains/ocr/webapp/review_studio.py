...
from backend.app.domains.ocr.annotator.anchor_helper import AnchorHelper
...
REAL_ANNOTATED_DIR = Path(
    "/home/daominhwysi/project/vietnamese-exam-seq-labelling/output/real_annotated"
)
RAW_DIR = WORKSPACE_DIR / "data" / "sequence_labelling_input_data"
REPORT_PATH = WORKSPACE_DIR / "backend" / "logs" / "real_annotated_review.json"
...
def run_deterministic_audit(
    ...
    # 3. Question and option structure
    ...
    # 4. Continuity
    cont_issues, cont_score = DeterministicAuditor.check_sequence_continuity(xml_content)
    issues.extend(cont_issues)

    # 5. Stimulus wrapping system tags
    ...
    # Weighted deterministic total score
    det_score = (
        syn_score * 0.25
        + proh_score * 0.15
        + q_score * 0.25
    ...
    all_metrics = {**q_metrics, **verb_metrics}
