...
def process_single_document(
    ...
    """
    Annotates a single document using the 48-64k chunker, skipping the linker agent,
    and saving both the merged version and per-chunk output files.
    """
    ...
    # --- Step 1: Chunker (48k - 64k tokens per chunk) ---
    ...
    worker = ParserAgentWorker(model=parser_model, provider=parser_provider, enable_validator=enable_validator)
    ...
    for idx, cres in enumerate(valid_chunk_results):
        ...
        chunk_file_info.append({
            ...
            "role_b_rating": cres.get("role_b_rating", "5/5"),
            "role_b_status": cres.get("role_b_status", "APPROVED"),
