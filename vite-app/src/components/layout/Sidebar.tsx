...
export function Sidebar({ activeTab, setActiveTab, examCount }: SidebarProps) {
  ...
  return (
    <aside className="sticky top-0 h-screen w-[270px] border-r border-border bg-sidebar flex flex-col justify-between p-3 select-none shrink-0 overflow-y-auto z-20">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between gap-2 px-2.5 py-2 rounded-md text-xs font-medium transition-colors ${
                  isActive
      ...
      {/* Footer Info */}
      <div className="p-2 bg-muted/40 rounded-lg border border-border/50 text-[11px] text-muted-foreground space-y-1">
        <div className="font-medium text-foreground">💡 Azota PDF OCR Engine</div>
        <p className="text-[10px] leading-tight">
          Hỗ trợ bóc tách đề thi tự động bằng BIO Sequence Tagging & PyMuPDF engine.
        </p>
      </div>
    </aside>
  );
