...
export function parseExamLanguage(
  text: string
): Omit<Exam, "id" | "created_at"> {
  ...
  const commitQuestion = () => {
    if (currentQ.stem || currentQ.options.length > 0) {
      const qId =
        currentQ.id ||
        `q_${questions.length + 1}_${Math.random().toString(36).substring(2, 8)}`
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed.toLowerCase().startsWith("duration:")) {
      duration_minutes =
        parseInt(line.substring(line.indexOf(":") + 1).trim(), 10) || 45
      continue
    }
          ...
          if (
            ...
            if (!currentQ.question_number) {
              const match = trimmed.match(
                /^(\**(?:Câu|Question|\d+)?\s*\d+\**[.:*]*)\s*(.*)/i
              )
              if (match) {
                currentQ.question_number = match[1]
                  .replace(/[.:*]/g, "")
                  .trim()
...
export function serializeExamLanguage(exam: Exam): string {
  ...
  for (const q of exam.questions) {
    if (
      q.stimulus_id &&
      q.stimulus_text &&
      !printedStimuli.has(q.stimulus_id)
    ) {
      text += `[Stimulus]\n`
