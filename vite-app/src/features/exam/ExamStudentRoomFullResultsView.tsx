...
function ResultOption({ opt, result, choiceStyle }: { opt: { label: string; text: string }; result: DetailedResult; choiceStyle: "radio" | "abcd" }) {
  ...
  if (isSelected) {
    if (result.is_correct) {
      ...
      dotColor = "bg-success"
      badgeClass = "border-success bg-success text-success-foreground font-extrabold"
    } else {
      optStyle =
        "border-transparent bg-destructive/5 text-destructive font-semibold"
      radioBorder = "border-destructive bg-destructive/5"
      dotColor = "bg-destructive"
      badgeClass = "border-destructive bg-destructive text-destructive-foreground font-extrabold"
    }
  } else if (isCorrectOption) {
    optStyle =
      "border-transparent bg-success/5 text-success"
    radioBorder = "border-success/30 bg-success/5"
    dotColor = "bg-success/40"
    badgeClass = "border-success bg-success/80 text-success-foreground font-extrabold"
  }
